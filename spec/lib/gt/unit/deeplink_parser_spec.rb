...
describe GT::DeeplinkParser do
  ...
  context "parse_deep" do
    it "should parse deep corecctly" do
      ...
      fake_em_http_request.stub(:get).and_return(
          mock_model("FakeEMHttpResonse", :error => false,
            :response_header => mock_model("FakeResponseHeader", :status => 200), :response => open(File.expand_path("../testdeepfiles/rant.html",__FILE__)).read))
    ...
    it "should not return bad url" do
      ...
      fake_em_http_request.stub(:get).and_return(
          mock_model("FakeEMHttpResonse", :error => false,
            :response_header => mock_model("FakeResponseHeader", :status => 200), :response => open(File.expand_path("../testdeepfiles/badlinks.html",__FILE__)).read))
        EventMachine::HttpRequest.stub(:new).and_return(fake_em_http_request)
    ...
    it "should not find something" do
      ...
      fake_em_http_request.stub(:get).and_return(
          mock_model("FakeEMHttpResonse", :error => false,
            :response_header => mock_model("FakeResponseHeader", :status => 200), :response => open(File.expand_path("../testdeepfiles/google.html", __FILE__)).read))
        EventMachine::HttpRequest.stub(:new).and_return(fake_em_http_request)
