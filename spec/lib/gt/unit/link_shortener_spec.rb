...
describe GT::Linkshortener do
  ...
  context "get or create short links for frames" do
    ...
    it "should return a short link that we have already" do
      ...
      r = GT::Linkshortener.get_or_create_shortlink(@frame, ["twitter"])
    ...
    it "should return error if we dont pass an array" do
      lambda { GT::Linkshortener.get_or_create_shortlink("test") }.should raise_error(ArgumentError)
    end
  ...
  context "get or create short links for rolls" do
    ...
    it "should return a short link that we have already" do
      ...
      r = GT::Linkshortener.get_or_create_shortlink(@roll, ["twitter"])
    ...
    it "should return error if we dont pass an array" do
      lambda { GT::Linkshortener.get_or_create_shortlink(@roll, "test") }.should raise_error(ArgumentError)
      lambda { GT::Linkshortener.get_or_create_shortlink(@roll) }.should raise_error(ArgumentError)
      lambda { GT::Linkshortener.get_or_create_shortlink(Factory.create(:user), "test") }.should raise_error(ArgumentError)
    end
  end
