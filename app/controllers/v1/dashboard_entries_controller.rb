class V1::DashboardEntriesController < ApplicationController  
  ...
  def find_entries_with_video
    ...
    selected_video = videos.select {|video| video.provider_id == provider_id and video.provider_name == provider_name}[0]
    selected_frames = frames.select {|frame| selected_video.id.to_s == frame.video_id.to_s}
    @entries = selected_frames
  end
